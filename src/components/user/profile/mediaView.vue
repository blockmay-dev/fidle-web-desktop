<template>
  <div>
    <div class="mb-4">
        ...
        >
          <div v-for="file in media.media" :key="file.id">
            <img
              ...
              width="100%"
            />
            <video v-else :src="file.file" playsinline loop></video>
          </div>
      ...
      <div v-if="mediaData.length === 0">You dont have any Media Files yet</div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    mediaData() {
      return this.$store.getters["user/allMediaFiles"];
    },
  },
};
</script>